...
>
    ...
    <form class="max-w-xl px-8 py-4 mx-auto bg-slate-300 rounded shadow"action="{{route('posts.store')}}" method="POST">
        @csrf
        @include('posts.form')
        ...
        <a class="text-sm font-semibold underline border-2 border-transparent rounded text-slate-600 focus:border-slate-500 focus:outline-none" href="{{route('posts.index')}}">Volver a listado</a>
        </div>
    </form>
